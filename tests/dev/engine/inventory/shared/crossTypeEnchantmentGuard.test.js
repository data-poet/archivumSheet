...
// project's architecture notes: accessories and magicGear both render
// generic .enchantment-* buttons (from the shared enchantments/render.js),
// and both wire them through createEnchantmentsHandlers' ownsFormKey guard.
// Nothing in the DOM distinguishes "this button belongs to an accessory"
// from "this button belongs to a piece of magic gear" — the ONLY thing
// preventing one type's handler from acting on the other type's instanceId
// is each handler's own findByInstanceId/getItems ownership check. This
// test proves that guard actually holds when both handlers are live at
// once, not just in isolation (accessories/events.test.js and
// magicGear/events.test.js each mock the OTHER type out of existence,
// which wouldn't catch a real collision).
